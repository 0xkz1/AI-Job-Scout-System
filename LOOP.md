...
Drawn in [loop-diagram.md](loop-diagram.md) — the whole night, and L2's gate on
its own. Two things in it are the design rather than the illustration, so they
are stated here as well:

**Every arrow out of this system ends at a person.** L1 stops at generated
documents; nothing sends them. L2 stops at a branch; nothing merges it. There is
no path to the outside world that does not pass through a human.

**L2 is one narrow arrow, not a licence to edit code.** It fires only when a
scraper ran clean and returned nothing, works on one scraper file in a throwaway
worktree, and its diff is discarded unread if it reaches anything else. It has
never fired in production — `git branch --list "loop/*"` is empty — because
nothing has broken. Dormant is the expected state, not a fault.

And one thing the diagram cannot show: the loop that has actually changed this
system runs on human attention rather than on cron. Something gets measured into
`_llm_stats.tsv` or into [STATE.md](STATE.md), a person reads it, and a change
follows. Every weight, gate and provider decision in the git log arrived that
way. The scheduled loops feed that loop; they do not replace it.
