...
def test_bare_name_pdf_is_frozen_once_a_new_version_exists(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(app, "PDF_DIR", pdf_dir)
    monkeypatch.setattr(app, "_PDF_VERSIONS_META", pdf_dir / ".pdf_versions.json")
    monkeypatch.setattr(app, "_md_to_pdf_bytes", lambda md_path: md_path.read_bytes())

    md_path = tmp_path / "TestCo_Role_CV.md"
    bare = pdf_dir / f"{app.APPLICANT_NAME_PREFIX}_{md_path.stem}.pdf"

    _write_cv(md_path, "version one content")
    p1, v1, is_new1 = app._convert_pdf_versioned(md_path)
    assert (v1, is_new1) == (1, True)
    assert p1 == bare
    v1_bytes = bare.read_bytes()

    _write_cv(md_path, "version two content, changed")
    p2, v2, is_new2 = app._convert_pdf_versioned(md_path)
    ...
    p3, v3, is_new3 = app._convert_pdf_versioned(md_path)
    ...
    p3b, v3b, is_new3b = app._convert_pdf_versioned(md_path)
